...
import com.md.utils.ExcelUtils;
...
public final class MethodInterceptor implements IMethodInterceptor {
	...
	public List<IMethodInstance> intercept(List<IMethodInstance> methods, ITestContext context) {
		List<Map<String, String>> data = ExcelUtils.getTestDetails(FrameworkConstants.getRunManagerSheet());
		List<IMethodInstance> result = new ArrayList<>();

		for (int i = 0; i < methods.size(); i++) {
			for (int j = 0; j < data.size(); j++) {
				if (methods.get(i).getMethod().getMethodName().equalsIgnoreCase(data.get(j).get("testname"))
